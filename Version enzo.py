...
class Simulation :
    ...
    def mise_a_jour_liste_voisin(self, poisson, i, p, angle_de_vision = 160):
...
def test_3():
    distance_seuil = 100; alpha_cohesion = 3; alpha_separation = 1000; alpha_alignement = 3; a_rng = 100
    r_cohesion = 175; r_separation = 60; r_alignement = 175
    N = 1500
    ...
    print(f"le parametre d'ordre à 2 secondes est de {nouvelle_simu.moyennage_parametre_ordre(800,700)}")

def generate_poissons():
    return [Poisson([rng.random()*500-250, rng.random()*500-250],
                    [rng.random()*400-200, rng.random()*400-200],
                    350)
            for _ in range(25)]
...
def meshgrid():
    ...
    for i in range(15):
        for j in range(15):
            ...
            sim = Simulation(liste, [], N, 0.01, distance_seuil, alpha_cohesion, alpha_separation, alpha_alignement, a_rng, r_cohesion[i], r_separation, r_alignement[j])
            sim.calcul_tableaux()
            print('/',end='')
